...
pred_path <- file.path(data_path, "na12878_sm_both_abundance.tsv")
...
pred_quant <- pred_quant %>% rename(target_id = transcript_id) %>% 
  filter(target_id != "None") %>% filter(tpm > 0) %>%
  mutate(tpm = tpm + 1e-5) %>% mutate(tpm = log2(tpm))
head(pred_quant)
...
g <- ggplot(quant_df, aes(x=actual, y=predicted)) + geom_point(alpha=0.05) + 
  ggtitle("StringTie vs. Ground Truth")
plot(g)
