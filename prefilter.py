...
def main():
    ...
    if args.jf_aligner is not None:
        print("jf_aligner binary path provided. Checking if the binary exists")
        if not os.path.isfile(args.jf_aligner + "/jf_aligner"):
            print("FATAL ERROR: jf_aligner binary not detected.")
            sys.exit()
    else:
        print("no binary path provided for jf_aligner")
    ...
    score_tbl = build_score_tbl(cmpt_tbl, reads_lst, cmpt_tbl_jf, reads_lst_jf, ms_tbl, cover_tbl, args.score_ratio)
