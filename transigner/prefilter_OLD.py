...
from transigner.commons import RED, GREEN, RESET, acore
import math
    ...
    with pysam.AlignmentFile(fn, 'rb') as fh:
        for brec in tqdm(fh):
            if brec.is_unmapped:
                ...
                if ti in aln_mat[qi]:
                    if curr_acore.ascore > aln_mat[qi][ti].ascore:
        ...
        for ti in aln_mat[qi]:
            ...
            if filter:
                ...
                if fp_dist < fp_thres or tp_dist < tp_thres:
                    continue
            # cmpt_score = acore.ascore / pri_score
            cmpt_score = math.exp((acore.ascore - pri_score) / 10) # debatable
